...
void txtTextboxCursorLocate () {
	...
	*_char = NULL; 
	txtTextboxCursor->pos_x = 1 + textboxMe->panel->pos_x + textboxMe->posX + str_width(strTemp, textboxMe->font) + textboxMe->borderSize + textboxMe->paddingX - textboxMe->spaceWidth / 2;
	if (*(_char - 1) == 32)
		txtTextboxCursor->pos_x += textboxMe->spaceWidth;
	txtTextboxCursor->pos_y = textboxMe->panel->pos_y + textboxMe->posY + txtTextboxCursor->skill_y * textboxMe->font->dy + textboxMe->borderSize + textboxMe->paddingY;
}
...
void textFromString (TEXTBOX *_tb, STRING *_strSource, int _index) {
	...
	while(str_len(_strSource) > 0) {
		...
		char _charOld = *_char;
		*_char = NULL;
		int _width = str_width(_strSource, _tb->font);
		for (; _width>_tb->width; _width=str_width(_strSource, _tb->font)) {
			_nextSpace -= 1;
			*_char = _charOld;
			_char -= 1;
			_charOld = *_char;
		...
		if(str_width(_strCurrent, _tb->font) + _width > _tb->width) {
		...
		case WM_CHAR:
			switch (_wParam) {
				...
				default:
					...
					_key[0] = _wParam;
					_key[1] = NULL;
					var _keyWidth = str_width(_str(_key), textboxMe->font);
					if (_keyWidth == 0) {
						if (_wParam != 32)
							break;
...
void txtTextboxCursorByMouse (TEXTBOX *_textbox) {
	...
	var _row = clamp(floor((mouse_pos.y - _minY) / _textbox->font->dy), 0, _textbox->sizeY - 1); 
	...
	if (_row != txtTextboxCursor->skill_y) {
		...
		var _width = str_width(strTemp, _textbox->font);
		char *_char = strTemp->chars + (int)txtTextboxCursor->skill_x - 1;
		if(*_char == 32)
			_width += _textbox->spaceWidth;
		for (; _width>_posX; _char-=1, txtTextboxCursor->skill_x-=1) {
			_width -= str_width(_str(_char), _textbox->font);
			if(*_char == 32)
				_width -= _textbox->spaceWidth;
			*_char = NULL;
		}
...
TEXTBOX *pan_textbox_create (PANEL *_pan, TEXTBOX_CONFIG *_config) {
	...
	_textbox->spaceWidth = str_width("a a", _textbox->font) - str_width("aa", _textbox->font);
	_textbox->width = _textbox->sizeX * _textbox->spaceWidth;
	_textbox->height = _textbox->sizeY * _textbox->font->dy;
