...
action props_switch()
{	
	if(nxs_layer == 1){my->skill1 = tiles[cm_XY[0]][cm_XY[1]].switchID;}
	if(nxs_layer == 2){my->skill1 = tilesB[cm_XY[0]][cm_XY[1]].switchID;}
	if(nxs_layer == 3){my->skill1 = tilesC[cm_XY[0]][cm_XY[1]].switchID;}
	
    // correct our Z position
    // and attach to the near by wall !
    my->z = WORLD_Z_POS;
    
    while(nxsGen_complete==false){wait(1);}
